...
#load the images
patient_image_1 = pydicom.read_file("IMG-0004-00001.dcm")
patient_image_2 = pydicom.read_file("IMG-0004-00002.dcm")

#display, image 2 overlaid onto image 1 with transparency
fig2 = plt.figure()
ax = fig2.add_subplot(111)
ax.imshow(patient_image_1.pixel_array, cmap='Greys_r')
ax.imshow(patient_image_2.pixel_array, alpha=0.3, cmap='Greys_r')
plt.title('Images 1 and 2 overlaid')
plt.show()
plt.savefig('images_1_and_2_overlaid.png'
...
def shift_image(vector, image, rotation=0):
    ...
    #floating.set_data(image)
    #fig.canvas.draw()
    return(rotated_image)

shift_image([20, 20], patient_image_1, -5)
