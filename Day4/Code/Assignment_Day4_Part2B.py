...
def displayImageWinLevel(image, window, level):
    ...
    plt.savefig("Tumour_regression_rotated.png", dpi=1200)
    plt.show()
...
for i in range(0, len(croped_img), 1): # this loop is designed to get i values from 0 to the number of images in the floating_list 
    croped_img[i] = croped_img[i][50:75, 303:326] # crop to the region of interest
...
plt.ylabel("Tumour area/%")
plt.title("Total Tumour Regression is {}% after 3rd treatment".format(reg))
plt.savefig("Total_Tumour_regression.png", dpi=1200)
plt.show()



'''
Please refer to these images for results:
Tumour_regression.png
Tumour_regrression_rotated.png
Total_tumour_regression.png


For XXIII (Testing of another ROI) use Assignemnt_Day4_Part2C'''
