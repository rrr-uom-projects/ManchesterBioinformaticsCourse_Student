...
for cFile in csvFiles:
    with open(cFile, mode = "r") as f:
        breathDataList.append(np.loadtxt(cFile, delimiter = ","))
...
#This runs through the breathDataList and plots each plot
#on its own axes. Probably a more elegant way to do this using subplot
for i in range(len(breathDataList)):
    xValues = breathDataList[i][0, :]
    yValues = breathDataList[i][1,:]
...
#Plots all 3 graphs                                                           
    ...
    plt.legend(loc = 4)
    plt.title("Spirometry results {}".format(str(csvFiles[i][:-4])))
